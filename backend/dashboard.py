...
try:
    ...
    AI_ENABLED = False
    print("⚠️ AI Agent not available. Install crewai and dependencies to enable.")
...
app.layout = html.Div([
    html.H1("E-Scooter Usage Dashboard (Chicago)", style={'textAlign': 'center', 'marginTop': '20px'}),

    html.Div([
        html.Div([
            html.Label("Select Vendor:", style={'fontWeight': 'bold', 'marginRight': '10px'}),
            dcc.Dropdown(
                id='vendor-dropdown',
        ...
        html.Div([
            html.Label("Unit System:", style={'fontWeight': 'bold', 'marginRight': '10px'}),
            dcc.RadioItems(
                id='unit-system',
                options=[
    ...
    html.Div(id='summary-cards', style={'margin': '20px'}),

    dcc.Graph(id='trips-per-day-chart'),
    dcc.Graph(id='provider-summary-chart'),
    dcc.Graph(id='avg-distance-hour-chart'),
    dcc.Graph(id='trip-location-map', config={'scrollZoom': True}),

    html.Hr(),
    html.H2("Weather Interaction Analysis 🌤️", style={'textAlign': 'center', 'marginTop': '40px'}),

    html.Div([
        html.Label("Select Weather Metric:", style={'fontWeight': 'bold', 'marginRight': '10px'}),
        dcc.Dropdown(
    ...
    html.Div(id='correlation-display', 
             style={'fontSize': '20px', 'margin': '20px', 'textAlign': 'center', 
                    'padding': '15px', 'backgroundColor': '#f0f0f0', 'borderRadius': '5px'}),

    html.Hr(),
    html.H2("🤖 AI Data Assistant", style={'textAlign': 'center', 'marginTop': '40px'}),
    html.P("Ask questions about the scooter data in natural language!", 
    ...
    dcc.Loading(
        ...
        type="default",
        children=[
            html.Div(id='ai-response-container', style={'margin': '20px'})
        ]
    ),
...
def update_summary_cards(selected_vendor, unit_system):
    ...
    for _, row in df.iterrows():
        ...
        cards.append(html.Div([
            html.H3(row['vendor'], style={'color': COLOR_MAP.get(row['vendor'], 'black')}),
            html.P(f"Total Trips: {int(row['total_trips']):,}"),
            html.P(f"Avg Duration: {row['avg_duration']:.1f} s"),
            html.P(f"Avg Distance: {distance_val:.2f} {distance_unit}"),
        ], style={
            'border': '2px solid #ddd',
            'borderRadius': '8px',
            'padding': '15px',
...
def update_provider_summary_chart(selected_vendor, unit_system):
    ...
    df['avg_duration_formatted'] = df['avg_duration'].apply(lambda x: f"{x:.3f} s")
...
def update_avg_distance_hour_chart(selected_vendor, unit_system):
    ...
    if unit_system == 'imperial':
        df['avg_distance'] = df['avg_distance'].apply(meters_to_miles)
        distance_label = 'Average Distance (mi)'
    else:
        df['avg_distance'] = df['avg_distance'].apply(meters_to_km)
